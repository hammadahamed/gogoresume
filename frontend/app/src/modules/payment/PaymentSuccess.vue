      ...
      <div v-else-if="verificationSuccess" class="space-y-5">
        ...
        <div class="bg-gray-50 rounded-lg p-5 my-5 text-left space-y-2">
          ...
          <p class="text-gray-800 text-sm">
            ...
            {{ formatDate(purchaseData.validUntil) }}
          </p>
          <p class="text-gray-800 text-sm">
...
const planMap = {
  pro_7_days: { name: "7 Days Pro", duration: "7 days" },
  pro_1_month: { name: "1 Month Pro", duration: "1 month" },
  pro_3_months: { name: "3 Months Pro", duration: "3 months" },
};
...
async function verifyPayment() {
  try {
    ...
    if (verificationResult.success) {
      purchaseData.value = verificationResult.purchase;
      planDetails.value = plans.find(
        (plan) => plan.id === verificationResult.purchase.planId
      );
      ...
      if (verificationResult.status === "processing") {
        ...
        toast.info(verificationResult.message || errorMessage);
      } else if (verificationResult.status === "not_found") {
        errorMessage =
          verificationResult.message ||
          "Payment not found. Please wait a few minutes for processing to complete.";
        toast.warning(errorMessage);
      } else if (verificationResult.error === "Payment failed") {
        errorMessage = "Payment failed. Please try again or contact support.";
        toast.error(errorMessage);
      } else {
        toast.error(verificationResult.error || errorMessage);
      }
